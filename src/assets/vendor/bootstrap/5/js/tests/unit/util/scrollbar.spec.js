...
describe('ScrollBar', () => {
  ...
  describe('hide - reset', () => {
    it('should adjust the inline padding of fixed elements which are full-width', (done) => {
      ...
      expect(getPaddingAttr(fixedEl)).toEqual(
        `${originalPadding}px`,
        'original fixed element padding should be stored in data-bs-padding-right'
      )
      expect(getPaddingAttr(fixedEl2)).toEqual(
        `${originalPadding2}px`,
        'original fixed element padding should be stored in data-bs-padding-right'
      )
      ...
      expect(currentPadding2).toEqual(
        expectedPadding2,
        'fixed element padding should be adjusted while opening'
      )
    ...
    it('should adjust the inline margin and padding of sticky elements', (done) => {
      ...
      expect(getMarginAttr(stickyTopEl)).toEqual(
        `${originalMargin}px`,
        'original sticky element margin should be stored in data-bs-margin-right'
      )
      expect(getMarginX(stickyTopEl)).toEqual(
        expectedMargin,
        'sticky element margin should be adjusted while opening'
      )
      expect(getPaddingAttr(stickyTopEl)).toEqual(
        `${originalPadding}px`,
        'original sticky element margin should be stored in data-bs-margin-right'
      )
      expect(getPaddingX(stickyTopEl)).toEqual(
        expectedPadding,
        'sticky element margin should be adjusted while opening'
      )
    ...
    it('should not adjust the inline margin and padding of sticky and fixed elements when element do not have full width', () => {
      ...
      expect(currentMargin).toEqual(
        originalMargin,
        "sticky element's margin should not be adjusted while opening"
      )
      expect(currentPadding).toEqual(
        originalPadding,
        "sticky element's padding should not be adjusted while opening"
      )
    ...
    describe('Body Handling', () => {
      it('should ignore other inline styles when trying to restore body defaults ', () => {
        ...
        expect(getPaddingX(document.body)).toEqual(
          scrollBarWidth,
