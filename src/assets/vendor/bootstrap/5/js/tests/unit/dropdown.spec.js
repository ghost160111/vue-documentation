...
describe('Dropdown', () => {
  ...
  describe('constructor', () => {
    ...
    it('should allow to pass config to Popper with `popperConfig` as a function', () => {
      ...
      const btnDropdown = fixtureEl.querySelector('[data-bs-toggle="dropdown"]')
      const getPopperConfig = jasmine.createSpy('getPopperConfig').and.returnValue({
        placement: 'left'
      })
      const dropdown = new Dropdown(btnDropdown, {
  ...
  describe('dispose', () => {
    it('should dispose dropdown', () => {
      ...
      expect(btnDropdown.addEventListener).toHaveBeenCalledWith(
        'click',
      ...
      expect(btnDropdown.removeEventListener).toHaveBeenCalledWith(
        'click',
        jasmine.any(Function),
        jasmine.any(Boolean)
      )
  ...
  describe('data-api', () => {
    ...
    it('should not use Popper in navbar', (done) => {
      ...
      btnDropdown.addEventListener('shown.bs.dropdown', () => {
        expect(dropdown._popper).toBeNull()
        expect(dropdownMenu.getAttribute('style')).toEqual(
          null,
          'no inline style applied by Popper'
        )
        done()
      })
    ...
    it('should remove "show" class if body if tabbing outside of menu, with multiple dropdowns', (done) => {
      ...
      triggerDropdownFirst.addEventListener('shown.bs.dropdown', () => {
        expect(triggerDropdownFirst.classList.contains('show')).toEqual(
          true,
          '"show" class added on click'
        )
        expect(fixtureEl.querySelectorAll('.dropdown-menu.show').length).toEqual(
          1,
      ...
      triggerDropdownLast.addEventListener('shown.bs.dropdown', () => {
        expect(triggerDropdownLast.classList.contains('show')).toEqual(
          true,
          '"show" class added on click'
        )
        expect(fixtureEl.querySelectorAll('.dropdown-menu.show').length).toEqual(
    ...
    it('should skip disabled element when using keyboard navigation', (done) => {
      ...
      triggerDropdown.addEventListener('shown.bs.dropdown', () => {
        ...
        expect(document.activeElement.classList.contains('disabled')).toEqual(
          false,
          '.disabled not focused'
        )
        expect(document.activeElement.hasAttribute('disabled')).toEqual(
          false,
          ':disabled not focused'
        )
        done()
    ...
    it('should skip hidden element when using keyboard navigation', (done) => {
      ...
      triggerDropdown.addEventListener('shown.bs.dropdown', () => {
        ...
        expect(document.activeElement.classList.contains('d-none')).toEqual(
          false,
          '.d-none not focused'
        )
        expect(document.activeElement.style.display).not.toBe('none', '"display: none" not focused')
        expect(document.activeElement.style.visibility).not.toBe(
          'hidden',
          '"visibility: hidden" not focused'
        )
    ...
    it('should ignore keyboard events for <input>s and <textarea>s within dropdown-menu, except for escape key', (done) => {
      ...
      triggerDropdown.addEventListener('shown.bs.dropdown', () => {
        ...
        expect(triggerDropdown.classList.contains('show')).toEqual(
          false,
    ...
    it('should propagate escape key events if dropdown is closed', (done) => {
      ...
      keydownEscape.key = 'Escape'
      const keyupEscape = createEvent('keyup', {
        bubbles: true
      })
    ...
    it('should close dropdown (only) by clicking inside the dropdown menu when it has data-attribute `data-bs-auto-close="inside"`', (done) => {
      ...
      const expectDropdownToBeOpened = () =>
        setTimeout(() => {
    ...
    it('should not close dropdown by clicking inside or outside the dropdown menu when it has data-attribute `data-bs-auto-close="false"`', (done) => {
      ...
      const expectDropdownToBeOpened = (shouldTriggerClick = true) =>
        setTimeout(() => {
          expect(dropdownToggle.classList.contains('show')).toEqual(true)
