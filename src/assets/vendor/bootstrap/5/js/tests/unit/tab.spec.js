...
describe('Tab', () => {
  ...
  describe('show', () => {
    ...
    it('show and shown events should reference correct relatedTarget', (done) => {
      ...
      secondTabTrigger.addEventListener('shown.bs.tab', (ev) => {
        ...
        expect(
          fixtureEl.querySelector('button:not(.active)').getAttribute('aria-selected')
        ).toEqual('false')
        done()
      })
  ...
  describe('data-api', () => {
    ...
    it('selecting a dropdown tab does not activate another', () => {
      ...
      expect(
        fixtureEl.querySelector('#nav1 .dropdown-toggle').classList.contains('active')
      ).toEqual(true)
      expect(
        fixtureEl.querySelector('#nav2 .dropdown-toggle').classList.contains('active')
      ).toEqual(false)
      expect(fixtureEl.querySelector('#nav2 .dropdown-item').classList.contains('active')).toEqual(
        false
      )
    })
