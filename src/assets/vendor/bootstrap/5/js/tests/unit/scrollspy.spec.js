...
describe('ScrollSpy', () => {
  ...
  const testElementIsActiveAfterScroll = ({
    elementSelector,
    targetSelector,
    contentEl,
    scrollSpy,
    ...
    const paddingTop = 5
    const scrollHeight =
      Math.ceil(contentEl.scrollTop + Manipulator.position(target).top) + paddingTop
  ...
  describe('constructor', () => {
    ...
    it('should generate an id when there is not one', () => {
      fixtureEl.innerHTML = ['<nav></nav>', '<div class="content"></div>'].join('')
  ...
  describe('dispose', () => {
    it('should dispose a scrollspy', () => {
      ...
      expect(divEl.addEventListener).toHaveBeenCalledWith(
        'scroll',
      ...
      expect(divEl.removeEventListener).toHaveBeenCalledWith(
        'scroll',
        jasmine.any(Function),
        jasmine.any(Boolean)
      )
  ...
  describe('jQueryInterface', () => {
    ...
    it('should create a scrollspy with given config', () => {
      ...
      expect(ScrollSpy.prototype.constructor).not.toHaveBeenCalledWith(div, {
        offset: 15
      })
